...
abstract contract ERC721Test is SymTest, Test {
    ...
    function check_ERC721NoBackdoor(bytes4 selector) public virtual {
        ...
        vm.prank(caller);
        bool success;
        if (uint32(selector) == 0xb88d4fde) {
            // TODO: support parameters of type bytes or dynamic arrays
    ...
    function _isApprovedOrOwner(address spender, uint256 tokenId) internal view returns (bool) {
        ...
        return (
            spender == owner || IERC721(token).isApprovedForAll(owner, spender)
                || IERC721(token).getApproved(tokenId) == spender
        );
    }

    function check_ERC721_transferFrom(
        address caller,
        address from,
        address to,
        address other,
